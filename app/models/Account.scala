...
class Accounts @Inject()(
   ...
   *
   * */
  def createAccount(account: Account): Future[Int] = db.run(accounts returning accounts.map(_.id) += account.serialize).recover {
  ...
  def updateBalance(id: Int, newBalance: Int): Future[Int] = db.run(accounts.filter(_.id === id).map(_.balance).update(newBalance)).recover {
   ...
   */
  def debit(accountId: Int, userId: Int, amount: Int): Future[Int] = {
  ...
  def transfer(senderAccountId: Int, receiverAccountId: Int, amount: Int): Future[Unit] = {
    val action = {
      for {
